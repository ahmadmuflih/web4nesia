...
class lbmn_WP_Import extends WP_Importer {
	...
	var $menu_item_titles_by_menu_id = array(); // LBMN Custom.
	...
	function import_start( $file ) {
		...
		$this->lbmn_get_existing_menu_items();
	...
	function get_authors_from_import( $import_data ) {
		if ( ! empty( $import_data['authors'] ) ) {
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					// Not important thing in our case.
	...
	/**
	 * Fill array $this->menu_item_titles_by_menu_id with data
	 * when importing process is starting (on import_start).
	 *
	 * @return void
	 */
	function lbmn_get_existing_menu_items() {
		$all_menus = wp_get_nav_menus();
		$menu_items_by_menu_id = array();

		foreach ( $all_menus as $menu) {
			$menu_items_by_menu_id[ $menu->term_id ] = array();
			$this_menu_items = wp_get_nav_menu_items( $menu->term_id );

			foreach ( $this_menu_items as $menu_item ) {
				$menu_items_by_menu_id[ $menu->term_id ][] = $menu_item->title;
			}
		}

		$this->menu_item_titles_by_menu_id = $menu_items_by_menu_id;
	}
	...
	function process_menu_item( $item ) {
		...
		// $menu_slug – is parent menu, not a single item slug.
		...
		if ( isset( $this->menu_item_titles_by_menu_id[ $menu_id ] )
				&& in_array( $item['post_title'], $this->menu_item_titles_by_menu_id[ $menu_id ] ) ){
			error_log('Item with title "' . $item['post_title'] . '" already exists in menu: ' . $menu_id );
			return;
		}
